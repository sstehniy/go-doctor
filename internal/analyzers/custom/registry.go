...
func resolveSelectors(selectors []string) ([]rule, error) {
	...
	for _, selector := range selectors {
		...
		if !ok {
			if !looksLikeCustomSelector(selector, groups) {
				continue
			}
			return nil, fmt.Errorf("unknown custom rule selector %q", selector)
		}
...
func selectorGroups() map[string][]rule {
	...
	for _, candidate := range registry {
		groups["custom"] = append(groups["custom"], candidate)
...
func looksLikeCustomSelector(selector string, groups map[string][]rule) bool {
	if selector == "custom" {
		return true
	}
